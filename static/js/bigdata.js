...
//加载录取人数、报到率
function regData(){
	$("#factTotalNum").text(18200+Math.floor(Math.random()*100+1));
	$("#totalNum").text(3850+Math.floor(Math.random()*100+1));
	$("#totalNumRate").text((1+Math.random()).toFixed(1)+'%');
...
data = [
    ...
    var schoolzb = geoCoordMap['山东'];
    for (var i = 0; i < data.length; i++) {
    	var da = [];
        var geoCoord = geoCoordMap[data[i].name];
        if (geoCoord) {
            da.push(schoolzb,geoCoord);
        }
        series.push({
	    	type: 'lines',
	        zlevel: 2,
	...
	var chart = echarts.init(document.getElementById('map'));
		chart.setOption({
	        ...
	        geo: {
		        ...
		        itemStyle: {
		            normal: {
		                areaColor: 'rgb(255, 255, 255,0.1)',
		                borderColor: 'rgb(132, 249, 253,0.7)',
						borderWidth:2
		            },
		            emphasis: {
...
function optionage(data){
	var optionAge = {
		    ...
		    xAxis: {
		        type: 'category',
		        data: ['12月1月','2月3月','4月5月','6月7月','8月9月','10月11月'],
		        axisLine: {
	 	            lineStyle: {
	 	                color: "#a0c0ef",
	 	                opacity:0.1,
