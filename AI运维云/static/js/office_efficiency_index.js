...
var callTime = "77单"; // 直接覆盖 callTime
$("#base-info").find("div[type='lineUpVal']").text(callTime || "无");
	...
	$("#base-info").find(".channel-total-score").text(channelTotalScore+"分");
 ...
 var myChart = echarts.init(document.getElementById('integral_echart'));
        ...
        // 强制设置地址
        $("#base-info").find(".channel-name").text("枣庄市薛城区XX园区");

	var popval2 = 0.1;  // 或其他值
	var percent = Math.round(popval2 * 100);  // 计算百分比
	var displayText = "全市前" + (percent || 30) + "%";  // 如果 percent 是 0，则显示 10%
$("#base-info").find(".popval").text(displayText);
    var rePortData={};
    //厅办理业务平均耗时
...
function loadChannelHandleDetail(data){
	var option = {
		    ...
		    yAxis: [{
		        ...
		        axisLine: {lineStyle: {color: '#57617B'}},
		        axisLabel: {margin: 10,textStyle: {fontSize: 12},textStyle: {color:'#181C41'},formatter:'{value}个'},
		    ...
		    series: [{
		        ...
		        name: '振动与噪声',type: 'line',smooth: true,lineStyle: { normal: {width: 2}},
		        yAxisIndex:0,
		        areaStyle: {
		            normal: {
		                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
		        ...
		        name: '压力',type: 'line',smooth: true,lineStyle: {normal: {width: 2}},
		        yAxisIndex:1,
		        areaStyle: {
		            normal: {
		                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
		        ...
		        name: '转速与负载',type: 'line',smooth: true,lineStyle: {normal: {width: 2}},
		        yAxisIndex:1,
		        areaStyle: {
		            normal: {
		                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
	...
	for(var key in staffHandleInfo){
		...
		var tr1=[
			...
			'<td colspan="3">',
					...
					'<div class="avg-time-label">|&nbsp;&nbsp;园区排名</div>',
			...
			'<tr>',
				'<td> ',
					'<div class="staff-cust-time">',
					   '<span style="font-size:15px;">总订单数量</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+'34笔</span>',
					'</div>',
				'</td>',
				'<td>',
					'<div class="staff-order-count">',
					    '<span style="font-size:15px;">已完成订单量</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+itemArr[0].orderNum+'笔</span>',
					'</div>',
				'</td>',
				'<td>',
			...
			'<td colspan="3"> ' +
'<div class="integral-label">车间积分</div> ' +
'<div class="integral-value">120分</div> ' +
'<div class="integral-label" style="width:60px;">|园区排名</div>' +
'<div class="integral-value" style="width:100px;">3/7</div>' +
'</td>'
		...
		if(itemArr.length>1){
			...
			tr2=[
				...
				'<tr>',
						'<td> ',
							   '<div class="staff-cust-time">',
					   '<span style="font-size:15px;">总订单数量</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+'56笔</span>',
					'</div>',
						'<td>',
							'<div class="staff-order-count">',
							    '<span style="font-size:15px;">已完成订单量</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+itemArr[1].orderNum+'笔</span>',
							'</div>',
						'</td>',
						'<td>',
							'<div class="staff-alarm">',
				...
				'<tr class="td-integral"> ',
					'<td colspan="3"> ',
				'<div class="integral-label">车间积分</div> ' +
					'<div class="integral-value">167分</div> ' +
					'<div class="integral-label" style="width:60px;">|园区排名</div>' +
					'<div class="integral-value" style="width:100px;">3/7</div>' +
					'</td>',
			]
	...
	for(var key in deviceInfo){
		...
		var tr1=[
				'<tr> ',                                                            
					...
					'</td> ',                                                           
				    '<td><div class="label-name">综合评分</div></td> ',                            
				    '<td rowspan="3" colspan="2" class="kuan-dai"> ',                   
				            '<div class="progress-label">最高温度 '+itemArr[0].up+'</div>',       
							'<div style="width:180px;height:15px;"> ',                         
								'<div class="progress" style="float:left;height:8px;width:180px;background-color:#383E60;">',
								     '<div class="progress-bar progress-bar-striped" style="min-width:0;width:'+itemArr[0].up.replace("℃","")+'%"></div>',
								'</div>',                                                    
							'</div> ',                                                      
				            '<div class="progress-label">最低温度 '+itemArr[0].down+'</div>',   
							'<div style="width:180px;height:15px;">',                      
								'<div class="progress" style="float:left;height:8px;width:180px;background-color:#383E60;">',
								   '<div class="progress-bar progress-bar-striped" style="min-width:0;width:'+itemArr[0].down.replace("℃","")+'%"></div>',
								'</div>',                                                    
							'</div>',                                                      
				     '</td>',                                                           
				 '</tr>',                                                           
				...
				'<tr>',                                                            
						'<td></td>',                                                       
						'<td><div class="labe-value">温度检测</div></td>',                 
						'<td><div class="labe-value">振动分析</div></td>',                
						'<td><div class="labe-value">磨损检测</div></td>',               
				'</tr>'
		];
		var tr2=[];
		if(itemArr.length>1){
			...
			tr2=[
				...
				'<tr> ',                                                            
					...
					'<td><div class="label-name">综合评分</div></td> ',                              
				    '<td rowspan="3" colspan="2" class="kuan-dai"> ',                   
				            '<div class="progress-label">最高温度 '+itemArr[1].up+'</div>',       
							'<div style="width:180px;height:15px;"> ',                         
								'<div class="progress" style="float:left;height:8px;width:180px;background-color:#383E60;">',
								     '<div class="progress-bar progress-bar-striped" style="min-width:0;width:'+itemArr[1].up.replace("℃","")+'%"></div>',
								'</div>',                                                    
							'</div> ',                                                      
				            '<div class="progress-label">最低温度 '+itemArr[1].down+'</div>',   
							'<div style="width:180px;height:15px;">',                      
								'<div class="progress" style="float:left;height:8px;width:180px;background-color:#383E60;">',
								   '<div class="progress-bar progress-bar-striped" style="min-width:0;width:'+itemArr[1].down.replace("℃","")+'%"></div>',
								'</div>',                                                    
							'</div>',                                                      
				     '</td>',                                                           
				 '</tr>',                                                           
				 ...
				 '<tr class="device-use">',                                         
						'<td></td>',                                                       
					'<td><div class="nei-cun-size"><span>61</span></div></td>' ,    
						'<td><div class="cpu-use"><span>1.4mm/s</span></div></td>',           
						'<td><div class="nei-cun-use"><span>167h</span></div></td>',       
				'</tr>',                                                           
				'<tr>',                                                            
					'<td></td>',                                                       
					'<td><div class="labe-value">温度检测</div></td>',                 
	...
	var option = {
		    ...
		    yAxis: [{
        		type: 'value',
 		        ...
 		        axisLine: {lineStyle: {color: '#57617B'}},
 		        axisLabel: {margin: 3,textStyle: {fontSize: 12},textStyle: {color:'#fff'},formatter:'{value}万件'},
 		        splitLine: {
 	                show: true,
 	                lineStyle:{
 	                    type:'dashed',
